...
const AddEditProduct = () => {
    ...
    const [product, setProduct] = useState({
        ...
        product_type: 'both', category_id: '', subcategory_id: '', supplier_id: '',
    ...
    useEffect(() => {
        const fetchDependencies = async () => {
            try {
                const [catRes, supRes] = await Promise.all([api.get('/categories'), api.get('/admin/suppliers')]);
            ...
            try {
                ...
                if (s && (s.id || s.name)) {
                    setProduct({
                        ...
                        product_type: s.product_type || 'both', category_id: s.category_id || '',
    ...
    const addSizeChip = (size) => {
        const current = product.size ? product.size.split(',').map(s => s.trim()).filter(Boolean) : [];
        if (!current.includes(size)) set('size', [...current, size].join(', '));
    };
    const removeSizeChip = (size) => {
        const current = product.size.split(',').map(s => s.trim()).filter(s => s && s !== size);
        set('size', current.join(', '));
    };
    ...
    const addColorChip = (color) => {
        setProduct(prev => {
            ...
            if (!current.includes(color)) return { ...prev, color: [...current, color].join(', ') };
            return prev;
        });
    };
        ...
        try {
            ...
            Object.entries(product).forEach(([key, val]) => {
                if (key === 'customization_flag' || key === 'for_schools_only') {
                    ...
                    // Skip sending color_images as it was, we'll send it as color_images_data
                    if (key !== 'color_images') {
            ...
            if (isEditing) {
                ...
                await api.post(`/admin/product/${id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            } else {
                await api.post('/admin/create-product', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {product.category_id && categories.find(c => c.id == product.category_id)?.children?.length > 0 && (
                                <div>
                                    ...
                                    <select className={inputCls} value={product.subcategory_id} onChange={e => {
                            ...
                            <div>
                                ...
                                <select required className={inputCls} value={product.supplier_id} onChange={e => set('supplier_id', e.target.value)}>
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200 min-h-[60px]">
                                ...
                                <input type="text" placeholder="Custom size..." value={customSizeInput}
                                    ...
                                    onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), customSizeInput.trim() && (addSizeChip(customSizeInput.trim()), setCustomSizeInput('')))}
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                        ...
                        {product.customization_flag && (
                            <div className="space-y-4">
                                ...
                                <div>
                                    <label className={labelCls}>Logo Placement Image</label>
                                    <div className="relative">
                                        <input type="file" className="absolute inset-0 opacity-0 cursor-pointer z-10"
                                            onChange={e => setLogoPlacementFile(e.target.files[0])} />
                                        <div className="h-24 border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center gap-2 text-slate-400 hover:border-indigo-400 transition-colors">
                                            <Upload size={16} />
                                            <span className="text-xs">{logoPlacementFile ? logoPlacementFile.name : 'Upload placement reference'}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
                        ...
                        {[
                            { label: 'Size Chart', ref: sizeChartFile, set: setSizeChartFile },
                            { label: 'Demo Image', ref: demoImageFile, set: setDemoImageFile },
                        ].map(m => (
                            <div key={m.label}>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="py-3 px-4 border border-slate-200 rounded-lg flex items-center gap-2 text-slate-400 hover:border-indigo-400 transition-colors text-xs">
                                        <Upload size={14} />
                                        {m.ref ? m.ref.name : 'Upload file'}
                                    </div>
                                </div>
                            </div>
                        ))}
